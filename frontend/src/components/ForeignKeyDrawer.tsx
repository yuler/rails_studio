...
export const ForeignKeyDrawer: React.FC<ForeignKeyDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-[450px] bg-zinc-900 border-l border-zinc-800 shadow-2xl z-50 flex flex-col animate-in slide-in-from-right duration-200">
      {/* Header */}
      <div className="p-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-950/60">
        <div className="flex items-center space-x-2">
          <Link2 size={16} className="text-blue-400" />
          <h2 className="text-sm font-semibold text-zinc-100 font-mono">
            {targetTable} <span className="text-zinc-500 font-normal">#{targetId}</span>
          </h2>
        </div>
        <div className="flex items-center space-x-2">
          <button
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-48 space-y-3">
            ...
            <p className="text-xs text-zinc-500 font-mono">Loading record details...</p>
          </div>
        ) : error ? (
          <div className="p-4 rounded-lg bg-red-950/40 border border-red-800/50 text-red-300 text-xs font-mono">
          ...
          <div className="space-y-4 font-mono text-xs">
            ...
            <div className="border border-zinc-800 rounded-lg overflow-hidden divide-y divide-zinc-800/60 bg-zinc-950/40">
              {Object.entries(record).map(([colName, value]) => {
                ...
                return (
                  <div key={colName} className="p-2.5 flex flex-col space-y-1 hover:bg-zinc-900/40 transition">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-1.5 text-zinc-400">
                        {isPk && <Key size={11} className="text-amber-400" />}
                        {fkInfo && <Link2 size={11} className="text-blue-400" />}
                        <span className="font-medium text-zinc-300">{colName}</span>
                        {colMeta && (
                      ...
                      <button
                        ...
                        title="Copy value"
                      >
                        {copiedKey === colName ? <Check size={12} className="text-emerald-400" /> : <Copy size={12} />}
                      </button>
                    </div>
                    ...
                    <div className="mt-0.5">
                      {value === null ? (
                        ...
                        <button
                          ...
                          className="inline-flex items-center space-x-1 px-2 py-0.5 rounded bg-blue-950/60 border border-blue-800/60 text-blue-300 hover:bg-blue-900/60 transition text-xs"
                        >
                          <Link2 size={10} />
                        ...
                        <span className={`px-1.5 py-0.5 rounded text-[11px] ${value ? 'bg-emerald-950 text-emerald-300' : 'bg-red-950 text-red-300'}`}>
                          {value ? 'true' : 'false'}
                        </span>
                      ) : typeof value === 'object' ? (
                        <pre className="p-2 bg-zinc-950 rounded text-[11px] text-zinc-300 overflow-x-auto border border-zinc-800/80">
                          {JSON.stringify(value, null, 2)}
                        </pre>
                      ) : (
                        <div className="text-zinc-200 break-all select-text font-mono">
                          {String(value)}
                        </div>
                      )}
                    </div>
